<template>
    <div class="update-question-container card shadow p-4 mx-auto">
      <h1 class="text-center text-primary">Update Question</h1>
  ...
  // Fetch the question data when the component is mounted
  onMounted(async () => {
    ...
    try {
      ...
      if (response && response.status === 200) {
        ...
        const correctIndex = question.value.possibleAnswers.findIndex(
          (answer) => answer.isCorrect
        );
        correctAnswerIndex.value = correctIndex;
      } else {
        errorMessage.value = 'Failed to fetch question data.';
      }
    } catch (error) {
  ...
  const submitUpdatedQuestion = async () => {
    try {
      ...
      const questionId = route.params.id;
      const response = await quizApiService.updateQuestion(questionId, question.value);
      if (response && response.status === 204) {
        ...
        errorMessage.value = 'Failed to update the question.';
      }
    } catch (error) {
